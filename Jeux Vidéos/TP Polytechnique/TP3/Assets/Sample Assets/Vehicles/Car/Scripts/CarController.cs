...
public class CarController : MonoBehaviour
{
	...
	// Nitro
	[SerializeField] private float nitroImpact = 2.0f; 								// The impulsion procured by the nitro
	public delegate void NoNitro();
	public NoNitro noNitro;															// Triggers when nitro is not being used
	
	// Jump
	[SerializeField] private float jumpForce = 800f;								// The car jump force
	private CarUserControlMP userControl;
	
	// Health
	[SerializeField] private GameObject[] obstacles;								// The obstacles that can harm the car
	[SerializeField] private float immobilizedTime = 1f;							// The time to immobilize when dead
	
	// Rubberbanding
	[SerializeField] private float rubberbandingBonus = 1.1f;						// The acceleration bonus of the rubberbanding
	[SerializeField] private int rubberbandingPositionLimit = 4;					// The position the car has to be behind to in order to have the rubberbanding bonus
	[SerializeField] private CheckpointManager checkpointManager;					// The checkpoint manager
	
	// Orientation in the air	
	[SerializeField] private float horizontalAirFactor = 1f;						// Horizontal axis rotation factor in the air
	[SerializeField] private float verticalAirFactor = 1f;							// Vertical axis rotation factor in the air
	[SerializeField] private float thirdAxisAirFactor = 1f;	
    ...
    private CarNitroManager nitroManager;											// The car's nitro manager
	private CarHealthManager healthManager;											// The car's health manager
	...
	private float score;															// The car's score
	private float nitro;															// The car's nitro fuel
	...
	void Awake ()
    {
		...
		nitroManager = GetComponent<CarNitroManager>();
		healthManager = GetComponent<CarHealthManager>();
		userControl = GetComponent<CarUserControlMP>();
        ...
        // Health : immobilize when dead
        healthManager.die += delegate() {
 			StartCoroutine("Die");
       	};
	}
	...
	public void Move (float steerInput, float accelBrakeInput, float thirdAxisInput, bool nitro, bool jump)
    {
		// lose control of engine if immobilized
		if (immobilized) accelBrakeInput = 0;
		
		if (nitro && nitroManager.Nitro > 0) {
			accelBrakeInput *= nitroImpact;
			nitroManager.ConsumeNitro();
		}
		else if (noNitro != null) {
			noNitro();
		}
		
		if (checkpointManager.IsBehind(this, rubberbandingPositionLimit)) {
			accelBrakeInput *= rubberbandingBonus;
		}
		...
		// Rotation in the air
		if (!anyOnGround) {
			RotateInTheAir(steerInput, accelBrakeInput, thirdAxisInput);
		}
	...
	void RotateInTheAir(float horizontal, float vertical, float thirdAxis) {
		transform.Rotate(vertical*verticalAirFactor, horizontal*horizontalAirFactor, thirdAxis*thirdAxisAirFactor);
	}
	...
	{
		if (IsAnObstacle(col.gameObject)) {
			healthManager.Damage();
		}
	}
	
	bool IsAnObstacle(GameObject go) {
		foreach (GameObject container in obstacles) {
			foreach (Transform child in container.GetComponentsInChildren<Transform>()) {
				if (child == go.transform && child != this.transform) {
					return true;
				}
			}
		}
