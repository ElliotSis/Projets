...
public class WaypointCircuit : MonoBehaviour {
	...
	public Transform getClosestPoint(Transform t){
		Transform res = Waypoints [0];
		float distMin = 99999999.0f;
		for(int i = 0; i < Waypoints.Length; i++){
			float aux = Vector3.Distance(t.position, Waypoints[i].position);
			if(aux < distMin){
				res = Waypoints[i];
				res.forward = Waypoints[(i+1)%Waypoints.Length].position - Waypoints[i].position;
				distMin = aux;
			} 
		}
		return res;
