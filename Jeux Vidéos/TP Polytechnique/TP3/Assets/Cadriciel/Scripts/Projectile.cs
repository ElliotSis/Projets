...
public class Projectile : MonoBehaviour {
	...
	void Awake() {
		...
		if (type == ProjectileType.red) {
			...
			if(checkpointManager.CarPositions[player.GetComponent<CarController>()].position != 1){
				foreach (Transform car in cars.transform){
					if(checkpointManager.CarPositions[car.gameObject.GetComponent<CarController>()].position == 1){
						target = car;
						break;
					}
				}
			} else {
				type = ProjectileType.green;
			}
		}
	}

	void OnCollisionEnter(Collision other) {
		if (type != ProjectileType.blue || target.transform == other.transform) {
			if (cars != null) 
				foreach (Transform car in cars.transform)
					try {
