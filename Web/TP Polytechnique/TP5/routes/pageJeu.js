...
router.get('/jeu/:pageId', function(req, res, next) {
    var send = true;
    var id = req.session.currentPageId ? req.session.currentPageId : req.params.pageId;
    var htmlPage = u.chain(fs.readdirSync('views/page'))
        // On récupère les sous-sections de la page
        .filter(function(file) {
             return file.indexOf(id + '_') == 0 && file.indexOf('~') < 0;
        })
    ...
    if (req.session.follow) {
        Joueur.findById(req.session.currentId, function(err, joueur) {
            if (err) {
                console.log(err);
            } else if (joueur) {
