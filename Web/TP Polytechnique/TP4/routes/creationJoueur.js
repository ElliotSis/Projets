...
router.post('/jeu/1', function(req, res) {
    ...
    var NB_DISCIPLINE = 5;
    if (disciplines.length != NB_DISCIPLINE) {
    ...
    if (u.isEmpty(erreursMsg)) {
        ...
        // Ajouter le joueur à la database
        var j = new Joueur();
        var avancement = new Avancement();

        j.habilete = joueur.habilete;
        j.endurance = joueur.endurance;
        j.pieceOr = joueur.pieceOr;
        j.disciplines = joueur.disciplines;
        j.armes = joueur.armes;
        j.objets = joueur.objets;
        j.objetsSpeciaux = joueur.objetsSpeciaux;

        avancement.pageActive = 1;
