...
const
  ...
  MOTSPROBABLES: array[1..16] of String = ('DANS','TAIT','ELLE','POUR','PLUS','MAIS','TOUT','VOUS','AVEC','NOUS','QUIL','DEUX','BIEN','CEST','SANS','DUNE');
  TAILLEMOTCLE = 4;
{$J+}
...
begin
	res := texte;
 	...
 	begin
 		for j := 0 to (length(texte) div 4) do 
 		begin
			res[i + 4*j] := dechiffrerCaractere(texte[i + 4*j], -ord('A') + ord(motCle[i]));
		end;
		dechiffrerTexte := res
	end;
end;
...
begin
	...
	begin
		if ord(motProbable[i]) - ord(texte[pos+i-1]) >= 0 then
			...
			difference := -ord(motProbable[i]) + ord(texte[pos+i-1]);
		res[i] := dechiffrerCaractere('A', difference);
	end;
	motCle := res
end;
...
begin
	...
	var texte, res : Ansistring;
begin
	texte := importer('cryptogramme.txt');
	writeln(attaqueMotPrb(texte, 4))
end.
